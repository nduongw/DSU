...
class ConstStyle6(nn.Module):
    ...
    def calculate_domain_distance(self, idx):
        ...
        for i in range(len(seen_domain_mean)):
            distance = wasserstein_distance_multivariate(unseen_domain_mean[0], unseen_domain_cov[0], seen_domain_mean[i], seen_domain_cov[i])
    ...
    def forward(self, x, domain, store_feature=False, apply_conststyle=False, is_test=False, cluster_idx=0):
        ...
        # style_feats = torch.hstack((mu, sig)).squeeze().detach().cpu().float()
        mu, sig = mu.detach(), sig.detach()
        x_normed = (x - mu) / sig
        
        if is_test:
            # dist = euclid_distance(style_feats, self.const_mean)
            # print(f'Distance: {dist}\n')
            # weight = sigmoid_increase(dist).to('cuda')
            # print(f'Weight: {weight}\n')
            # weight = torch.reshape(weight, (x.size(0), 1, 1, 1)).float()
            ...
            # beta = const_std * weight + (1 - weight) * sig
            # gamma = const_mean * weight + (1 - weight) * mu
