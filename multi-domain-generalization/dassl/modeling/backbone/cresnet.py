...
class CResNet(Backbone):
    ...
    def featuremaps(self, x, domain, store_feature=False, apply_conststyle=False, is_test=False, apply_rate=0.0):
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)
        ...
        if self.cfg.TRAINER.CONSTSTYLE.NUM_CONSTSTYLE >= 3:
            x = self.conststyle[2](x, domain, store_feature=store_feature, apply_conststyle=apply_conststyle, is_test=is_test, apply_rate=apply_rate)
        x = self.layer3(x)
        if self.cfg.TRAINER.CONSTSTYLE.NUM_CONSTSTYLE >= 4:
            x = self.conststyle[3](x, domain, store_feature=store_feature, apply_conststyle=apply_conststyle, is_test=is_test, apply_rate=apply_rate)
        x = self.layer4(x)
        if self.cfg.TRAINER.CONSTSTYLE.NUM_CONSTSTYLE >= 5:
            x = self.conststyle[4](x, domain, store_feature=store_feature, apply_conststyle=apply_conststyle, is_test=is_test, apply_rate=apply_rate)
        return x
        
    def forward(self, x, domain, store_feature=False, apply_conststyle=False, is_test=False, apply_rate=0.0):
        f = self.featuremaps(x, domain, store_feature=store_feature, apply_conststyle=apply_conststyle, is_test=is_test, apply_rate=apply_rate)
