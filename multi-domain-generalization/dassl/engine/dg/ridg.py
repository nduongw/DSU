...
class RIDG(TrainerX):
    def __init__(self, cfg, args):
        ...
        write_test_data = ['epoch', 'class', 'hindex1', 'hindex2', 'hindex3', 'hindex4', 'hindex5', 'value1', 'value2', 'value3', 'value4', 'predicted_class']
        csvwriter_train.writerow(write_train_data)
        csvwriter_test.writerow(write_test_data)
        f_train.close()
        f_test.close()
        
    def forward_backward(self, batch, momentum):
        ...
        for i in range(classes.shape[0]):
            ...
            if self.init[classes[i]]:
                ...
                self.rational_bank[classes[i]] = (1 - momentum) * self.rational_bank[classes[i]] + momentum * rational_mean
        ...
        print(f'cross entropy loss: {ce_loss} | rational loss: {self.cfg.TRAINER.RIDG.REG * loss_rational}')
    ...
    def run_epoch(self):
        momentum = max(self.cfg.TRAINER.RIDG.MOMENTUM, 0.1 - self.cfg.TRAINER.RIDG.MOMENTUM * (self.epoch / 10))
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            loss_summary = self.forward_backward(batch, momentum)
